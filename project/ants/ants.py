    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9 for BodyguardAnt
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while cur_place != hive and not cur_place.bees:
            cur_place = cur_place.entrance
            range += 1
        if cur_place == hive or range > self.max_range:
            return None
        return random_or_none(cur_place.bees)
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        ...
        self.digesting=self.digesting_turns
        # END Problem 6
...
# BEGIN Problem 13
class QueenAnt(ScubaAnt):  # You should change this line
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        ScubaAnt.__init__(self)
        self.is_queen=self.Queen
        QueenAnt.Queen=not QueenAnt.Queen
        self.ants_lst=[]
        # END Problem 13
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.is_queen:
            ScubaAnt.action(self,colony)
            cur_place=self.place
            while cur_place.exit:
                cur_place=cur_place.exit
                if isinstance(cur_place.ant,BodyguardAnt):
                    cur_place.ant.damage*=2
                    if cur_place.ant.ant and cur_place.ant.ant not in self.ants_lst:
                        cur_place.ant.ant.damage*=2
                        self.ants_lst.append(cur_place.ant.ant)
                    self.ants_lst.append(cur_place.ant)
                if cur_place.ant not in self.ants_lst and cur_place.ant:
                    cur_place.ant.damage*=2
                    self.ants_lst.append(cur_place.ant)
