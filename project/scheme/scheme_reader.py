...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        return Pair('quote', Pair(scheme_read(src), nil)) #为什么不把scheme_read放在第二位？配合后面的do_quote_form
        # END PROBLEM 6
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            src.pop_first()  #如果不pop掉)的话，递归到最后，只要有一个)，缺少)结尾的tail都能返还nil
            return nil
            # END PROBLEM 1
        else:
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"#递归：每read一次就pop一次，首位返还read的结果后，次位对src下一位tail，返还pair或nil
