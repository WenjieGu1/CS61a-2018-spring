...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    #expr都是pair类。eval赋值过程：1.数字或者name直接赋值；

    #2.特殊形式按照特殊的计算，
    #lambda定义这种是第一次eval先实例化为procedure（do_XXX_form），
    #然后第二次eval时才能作为operator代入参数；

    #3.一般形式如（+ 1 2），先把operands和operator都赋值，
    #再把operator应用到operand中，operator就是procedure类。

    #tips：将实例化的lambda视作一个operator。
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        return SPECIAL_FORMS[first](rest, env) #speical form的expr.first（开头符号）不参与到后面的函数
    else:
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operator=scheme_eval(first,env)
        validate_procedure(operator)
        oprerands=rest.map(lambda rest: scheme_eval(rest,env))
        return scheme_apply(operator,oprerands,env)
        # END PROBLEM 4
...
def scheme_apply(procedure, args, env):
    ...
    else:                   #lambda和mu是没有apply method的，
                            #但是有make_call_frame将形参和args绑定在新env中，再按照body赋值
...
class Frame(object):
    ...
    def __repr__(self):
        ...
        return '<{{{0}}} -> {1}>'.format(', '.join(s), repr(self.parent))   #str.join(sequence):返回通过指定字符连接序列中元素后生成的新字符串
    ...
    def make_child_frame(self, formals, vals):
        ...
        formals, vals=pair_to_list(formals), pair_to_list(vals)
        for formal, val in zip(formals, vals):
            new_frame.define(formal, val)
        return new_frame
        # END PROBLEM 10
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        of values, for a lexically-scoped call evaluated in environment ENV.把形参和args绑定在子env中"""
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
...
# the environment in which the form is to be evaluated. 接受的expr没有开头符号
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        name, formals, body=target.first, target.rest, expressions.rest.first
        lambda_expr=Pair(formals, Pair(body, nil))
        env.define(name, do_lambda_form(lambda_expr,env))
        return name
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    >>> env = create_global_frame()
    >>> do_quote_form(read_line("((+ x 2))"), env) #read之后 ((+ x 2)) length=1
...
def do_lambda_form(expressions, env):
    """Evaluate a lambda form.由上一步eval一个定义的lambda函数而来，在此赋值lambda形式的实例
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        validate_form(clause, 1) #至少有一个元素
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        formal,val=clause.first,scheme_eval(clause.rest.first, env) 
        names, values=Pair(formal, names), Pair(val, values)
